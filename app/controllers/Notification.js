(function () {
    ...
    var configApp = require('../../config/app')[process.env.NODE_ENV || 'production'];
    ...
    function sendPushNotification( req, res, next ){
       ...
       getDbIns.then(function (db) {
	       ...
	       db.collection("users").find({_id:userId}).toArray(function (err,users){
				...
				// notify.badge = 1;
				notify.sound = "ringtone.mp3";
				notify.alert = "You have received broadcast call";
				notify._key=userId;
           ...
           try {
					...
					if (apnConnection) {
                                 ...
                                 if(deviceToken1 && deviceToken2 && deviceToken3 && deviceToken4){
                   	     var deviceToken = [deviceToken1,deviceToken2,deviceToken3,deviceToken4];
                    	 apnConnection.send(notify, deviceToken ).then( (result) => {    
						 
						res.json({"status":"1", "message":"Notification 1,2,3,4 Send Successfully"});
                             ...
                             if(deviceToken1 && deviceToken2 && deviceToken3){
                   	     var deviceToken = [deviceToken1,deviceToken2,deviceToken3];
                    	 apnConnection.send(notify, deviceToken ).then( (result) => {    
						 
						res.json({"status":"1", "message":"Notification 1,2,3 Send Successfully"});
						...
						if(deviceToken1 && deviceToken2){
                   	     var deviceToken = [deviceToken1,deviceToken2];
                    	 apnConnection.send(notify, deviceToken ).then( (result) => {    
						 
						res.json({"status":"1", "message":"Notification 1,2 Send Successfully"});
						...
						res.json({"status":"1", "message":"Notification 1 Send Successfully"});
						...
						var gotres=JSON.parse(JSON.stringify(result));
						console.log(gotres.failed);  
						res.json({"status":"1", "message":"Notification 2 Send Successfully"});
              ...
              if(deviceToken3){
						...
						var gotres=JSON.parse(JSON.stringify(result));
						console.log(gotres.failed);  
						res.json({"status":"1", "message":"Notification 3 Send Successfully"});
              ...
              if(deviceToken4){
						...
						var gotres=JSON.parse(JSON.stringify(result));
						console.log(gotres.failed);  
						res.json({"status":"1", "message":"Notification 4 Send Successfully"});
